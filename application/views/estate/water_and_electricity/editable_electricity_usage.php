...
<div class="card">
    <div class="card-body">
        <br /> <br />
        <h3 class="card-title">Form to Update Electricity consumption for month <b><i><?= $data['month'].' , '.$data['year']; ?></i></b></h3>
        <form id="edit_usage_form" method="post">
            <?php
            ...
            <table class="table table-bordered border-primary">
                ...
                <tbody>
                    ...
                    $current_date = $current_data->recorded_date;
                    foreach($locations as $loc){ 
                        $prev_date = "";
                        $prev_units = "";
                        $current_unit = "";
                        $pre = $this->estate->prev_reading($loc->id, $prev_month, $year); 
                        $current = $this->estate->current_reading($loc->id,$data['month'],$data['year']);
                        if(!empty($pre)){
                            $prev_date = $pre->recorded_date;
                            $prev_units = $pre->units_recorded;
                        } 
                        if(!empty($current)){
                            $current_unit = $current->units_recorded;
                        } ?>
                        <tr>
                            ...
                            <td>
                            	<input type="date" name="<?= $loc->id . '_date'; ?>" class="form-control" value="<?= $current_date; ?>">
                            </td>
                            <?php
                                $Used_units = "";
                                if($prev_units != "" && $current_unit != ""){
                                    $diff = $current_unit-$prev_units;
                                    if($diff < 0){
                                        $Used_units = "";
                                    }else{
                                        $Used_units = round($diff,1);
                                    }
                                }
                            ?>
                            <td rowspan="2" class="text-center" id="<?= $loc->id.'_used_units_td'; ?>"><?= $Used_units; ?></td>
                        </tr>
                        <tr>
                            <td>Units</td>
                            <td><input type="number" class="form-control" value="<?= $prev_units; ?>" readonly></td>
                            <td><input type="number" name="<?= $loc->id.'_units'; ?>" class="form-control" value="<?= $current_unit; ?>" onchange="calculateUsedUnits(this,<?= $loc->id; ?>)"></td>
                        </tr>
                    <?php } ?>
                </tbody>
            </table>
            <center>
                ...
                <button type="button" class="btn btn-primary" onclick="update_electricity_usage()">Update</button>
            </center>
        </form>
    </div>
</div>
